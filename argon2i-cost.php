...
<html>
	<head>
		...
		<title>Argon2i Cost Calculator by @RomelSan</title>
		<link rel="stylesheet" href="skeleton.css">
	</head>
	<body>
	
	<h1>Argon2i Cost Calculator</h1>
		...
		do 
			{
				...
				$start = microtime(true);
				//$hash = password_hash($password, PASSWORD_ARGON2I, array('time_cost' => $cost)); // Default Test
				$hash = password_hash($password, PASSWORD_ARGON2I, array('time_cost' => $cost, 'threads' => $threads, 'memory_cost' => $memory));
				$time = round((microtime(true) - $start) * 1000);
